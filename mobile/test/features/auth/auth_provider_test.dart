...
void main() {
  ...
  group('register', () {
    test('success sets otp step and registered phone', () async {
    ...
    test('DioException sets parsed error message', () async {
      when(mockAuthService.register(phone: '963900000001')).thenThrow(
        createDioException(
          statusCode: 422,
          data: {'error': 'رقم الجوال مسجل مسبقاً'},
        ),
      );
    ...
    test('connection timeout sets network error', () async {
      when(mockAuthService.register(phone: '963900000001')).thenThrow(
      ...
      expect(authNotifier.state.isLoading, false);
      expect(authNotifier.state.error, contains('تعذر الاتصال بالخادم'));
    });

    test('connection error sets local server hint', () async {
      when(mockAuthService.register(phone: '963900000001')).thenThrow(
  ...
  group('createPin', () {
    test('success creates pin and stores token', () async {
      ...
      when(mockAuthService.setToken('auth-token-123'))
          .thenAnswer((_) async => {});
  ...
  group('loginWithPin', () {
    test('success sets authenticated and home step', () async {
      ...
      when(mockAuthService.setToken('login-token'))
          .thenAnswer((_) async => {});
