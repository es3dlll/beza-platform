...
├── 10-service-layer-refund.md       # RefundService كامل
├── 11-events-and-listeners.md       # DealCancelled + مستمعيه
├── 12-notification-system.md        # FCM + إشعارات الاسترجاع
...
| العنصر | القيمة |
|--------|--------|
| اسم العملية | إلغاء صفقة + استرجاع (Admin) |
| الأولوية | P1 (عالية) |
| API | `POST /api/v1/admin/deals/{id}/cancel` |
| Controller | `AdminDealController@cancel` |
| Service | `DealService` / `RefundService` |
| Event | `DealCancelled` |
| Listener | `SendRefundNotification` |
| DB Tables | deals, deal_investments, wallets, transactions |
| صلاحية | Admin فقط |
| Flutter Screen | `AdminDealCancelScreen` |
| React Page | `AdminDealCancelPage` |
