# 17 - تطبيق React (React Implementation) - التحويل عبر QR (QR Transfer)
...
export function useQRTransfer() {
  ...
  const submit = useCallback(async ({ amount, currency, pin }) => {
    ...
    try {
      const response = await api.post('/transfer/qr-generate', {
        amount, currency, pin,
      });
...
## QRTransferForm Component

```jsx
// components/QRTransfer/QRTransferForm.jsx
import React, { useState } from 'react';

export default function QRTransferForm({ onSubmit, loading, error }) {
...
```jsx
// pages/QRTransferPage.jsx
import React from 'react';
import QRTransferForm from '../components/QRTransfer/QRTransferForm';
import { useQRTransfer } from '../hooks/useQRTransfer';

export default function QRTransferPage() {
  const { loading, success, error, transaction, newBalance, submit, reset } = useQRTransfer();
  ...
  return (
    <div className="page">
      <h1>التحويل عبر QR</h1>
      <QRTransferForm onSubmit={submit} loading={loading} error={error} />
    </div>
  );
}
```
