...
```php
<?php
// tests/Feature/QRTransferTest.php
...
class QRTransferTest extends TestCase
{
    ...
    {
        $response = $this->withToken($this->token)->postJson('/transfer/qr-generate', [
            'amount'   => 100.00,
            'currency' => 'USD',
            'pin'      => '1234',
        ]);
    ...
    {
        $response = $this->postJson('/transfer/qr-generate', [
            'amount'   => 100.00,
            'currency' => 'USD',
            'pin'      => '1234',
        ]);
...
```bash
php artisan test --filter=QRTransferTest
