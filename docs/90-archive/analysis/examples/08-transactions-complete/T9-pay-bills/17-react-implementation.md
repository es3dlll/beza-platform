# 17 - تطبيق React (React Implementation) - دفع الفواتير (Pay Bills)
...
## PayBillsForm Component

```jsx
// components/PayBills/PayBillsForm.jsx
import React, { useState } from 'react';

export default function PayBillsForm({ onSubmit, loading, error }) {
...
```jsx
// pages/PayBillsPage.jsx
import React from 'react';
import PayBillsForm from '../components/PayBills/PayBillsForm';
import { usePayBills } from '../hooks/usePayBills';

export default function PayBillsPage() {
  const { loading, success, error, transaction, newBalance, submit, reset } = usePayBills();
  ...
  return (
    <div className="page">
      <h1>دفع الفواتير</h1>
      <PayBillsForm onSubmit={submit} loading={loading} error={error} />
    </div>
  );
}
```
