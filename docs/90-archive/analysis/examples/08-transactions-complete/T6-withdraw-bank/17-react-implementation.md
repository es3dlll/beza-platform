# 17 - تطبيق React (React Implementation) - سحب بنكي (Bank Withdrawal)
...
user-frontend/src/
├── pages/
│   └── BankWithdrawalPage.jsx
├── components/
│   └── BankWithdrawal/
│       └── BankWithdrawalForm.jsx
├── hooks/
│   └── useBankWithdrawal.js
└── services/
    └── api.js
```
...
export function useBankWithdrawal() {
  ...
  const submit = useCallback(async ({ amount, currency, pin }) => {
    ...
    try {
      const response = await api.post('/withdraw/bank', {
        amount, currency, pin,
      });
...
## BankWithdrawalForm Component

```jsx
// components/BankWithdrawal/BankWithdrawalForm.jsx
import React, { useState } from 'react';

export default function BankWithdrawalForm({ onSubmit, loading, error }) {
...
```jsx
// pages/BankWithdrawalPage.jsx
import React from 'react';
import BankWithdrawalForm from '../components/BankWithdrawal/BankWithdrawalForm';
import { useBankWithdrawal } from '../hooks/useBankWithdrawal';

export default function BankWithdrawalPage() {
  const { loading, success, error, transaction, newBalance, submit, reset } = useBankWithdrawal();
  ...
  return (
    <div className="page">
      <h1>سحب بنكي</h1>
      <BankWithdrawalForm onSubmit={submit} loading={loading} error={error} />
    </div>
  );
}
```
