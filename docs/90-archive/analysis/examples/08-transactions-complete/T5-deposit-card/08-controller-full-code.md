...
```php
<?php
// app/Http/Controllers/Api/CardDepositController.php
...
use App\Services\CardDepositService;
use Illuminate\Http\JsonResponse;

class CardDepositController extends Controller
{
    public function __construct(
        private readonly CardDepositService $service
    ) {}

    public function execute(CardDepositRequest $request): JsonResponse
    {
        $user = $request->user();
...
use App\Http\Controllers\Api\CardDepositController;

Route::middleware(['auth:api', 'throttle:30,1'])->group(function () {
    Route::post('/deposit/card', ...)
});
```
