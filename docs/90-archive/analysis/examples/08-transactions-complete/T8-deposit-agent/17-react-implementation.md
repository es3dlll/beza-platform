# 17 - تطبيق React (React Implementation) - إيداع وكيل (Agent Deposit)
...
user-frontend/src/
├── pages/
│   └── AgentDepositPage.jsx
├── components/
│   └── AgentDeposit/
│       └── AgentDepositForm.jsx
├── hooks/
│   └── useAgentDeposit.js
└── services/
    └── api.js
...
export function useAgentDeposit() {
  ...
  const submit = useCallback(async ({ amount, currency, pin }) => {
    ...
    try {
      const response = await api.post('/deposit/agent', {
        amount, currency, pin,
      });
...
## AgentDepositForm Component

```jsx
// components/AgentDeposit/AgentDepositForm.jsx
import React, { useState } from 'react';

export default function AgentDepositForm({ onSubmit, loading, error }) {
...
```jsx
// pages/AgentDepositPage.jsx
import React from 'react';
import AgentDepositForm from '../components/AgentDeposit/AgentDepositForm';
import { useAgentDeposit } from '../hooks/useAgentDeposit';

export default function AgentDepositPage() {
  const { loading, success, error, transaction, newBalance, submit, reset } = useAgentDeposit();
  ...
  return (
    <div className="page">
      <h1>إيداع وكيل</h1>
      <AgentDepositForm onSubmit={submit} loading={loading} error={error} />
    </div>
  );
}
```
