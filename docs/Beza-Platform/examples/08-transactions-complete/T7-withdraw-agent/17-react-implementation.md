# 17 - تطبيق React (React Implementation) - سحب وكيل (Agent Withdrawal)
...
user-frontend/src/
├── pages/
│   └── AgentWithdrawalPage.jsx
├── components/
│   └── AgentWithdrawal/
│       └── AgentWithdrawalForm.jsx
├── hooks/
│   └── useAgentWithdrawal.js
└── services/
    └── api.js
```
...
```javascript
// hooks/useAgentWithdrawal.js
import { useState, useCallback } from 'react';
import api from '../services/api';

export function useAgentWithdrawal() {
  ...
  const submit = useCallback(async ({ amount, currency, pin }) => {
    ...
    try {
      const response = await api.post('/withdraw/agent', {
        amount, currency, pin,
      });
...
## AgentWithdrawalForm Component

```jsx
// components/AgentWithdrawal/AgentWithdrawalForm.jsx
import React, { useState } from 'react';

export default function AgentWithdrawalForm({ onSubmit, loading, error }) {
...
```jsx
// pages/AgentWithdrawalPage.jsx
import React from 'react';
import AgentWithdrawalForm from '../components/AgentWithdrawal/AgentWithdrawalForm';
import { useAgentWithdrawal } from '../hooks/useAgentWithdrawal';

export default function AgentWithdrawalPage() {
  const { loading, success, error, transaction, newBalance, submit, reset } = useAgentWithdrawal();
  ...
  return (
    <div className="page">
      <h1>سحب وكيل</h1>
      <AgentWithdrawalForm onSubmit={submit} loading={loading} error={error} />
    </div>
  );
}
```
