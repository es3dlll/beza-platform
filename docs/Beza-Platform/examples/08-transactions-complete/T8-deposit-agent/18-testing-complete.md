...
```php
<?php
// tests/Feature/AgentDepositTest.php
...
class AgentDepositTest extends TestCase
{
    ...
    {
        $response = $this->withToken($this->token)->postJson('/deposit/agent', [
            'amount'   => 100.00,
            'currency' => 'USD',
            'pin'      => '1234',
        ]);
    ...
    {
        $response = $this->postJson('/deposit/agent', [
            'amount'   => 100.00,
            'currency' => 'USD',
            'pin'      => '1234',
        ]);
...
```bash
php artisan test --filter=AgentDepositTest
