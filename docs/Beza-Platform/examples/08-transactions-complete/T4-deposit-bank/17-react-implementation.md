# 17 - تطبيق React (React Implementation) - إيداع بنكي (Bank Deposit)
...
export function useBankDeposit() {
  ...
  const submit = useCallback(async ({ amount, currency, pin }) => {
    ...
    try {
      const response = await api.post('/deposit/bank', {
        amount, currency, pin,
      });
...
## BankDepositForm Component

```jsx
// components/BankDeposit/BankDepositForm.jsx
import React, { useState } from 'react';

export default function BankDepositForm({ onSubmit, loading, error }) {
...
```jsx
// pages/BankDepositPage.jsx
import React from 'react';
import BankDepositForm from '../components/BankDeposit/BankDepositForm';
import { useBankDeposit } from '../hooks/useBankDeposit';

export default function BankDepositPage() {
  const { loading, success, error, transaction, newBalance, submit, reset } = useBankDeposit();
  ...
  return (
    <div className="page">
      <h1>إيداع بنكي</h1>
      <BankDepositForm onSubmit={submit} loading={loading} error={error} />
    </div>
  );
}
```
