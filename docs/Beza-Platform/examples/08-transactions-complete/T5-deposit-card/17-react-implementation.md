# 17 - تطبيق React (React Implementation) - إيداع بطاقة ائتمانية (Card Deposit)
...
export function useCardDeposit() {
  ...
  const submit = useCallback(async ({ amount, currency, pin }) => {
    ...
    try {
      const response = await api.post('/deposit/card', {
        amount, currency, pin,
      });
...
## CardDepositForm Component

```jsx
// components/CardDeposit/CardDepositForm.jsx
import React, { useState } from 'react';

export default function CardDepositForm({ onSubmit, loading, error }) {
...
```jsx
// pages/CardDepositPage.jsx
import React from 'react';
import CardDepositForm from '../components/CardDeposit/CardDepositForm';
import { useCardDeposit } from '../hooks/useCardDeposit';

export default function CardDepositPage() {
  const { loading, success, error, transaction, newBalance, submit, reset } = useCardDeposit();
  ...
  return (
    <div className="page">
      <h1>إيداع بطاقة ائتمانية</h1>
      <CardDepositForm onSubmit={submit} loading={loading} error={error} />
    </div>
  );
}
```
