# 17 - تطبيق React (React Implementation) - طلب المال (Request Money)
...
user-frontend/src/
├── pages/
│   └── RequestMoneyPage.jsx
├── components/
│   └── RequestMoney/
│       └── RequestMoneyForm.jsx
├── hooks/
│   └── useRequestMoney.js
└── services/
    └── api.js
...
```jsx
// components/RequestMoney/RequestMoneyForm.jsx
import React, { useState } from 'react';

export default function RequestMoneyForm({ onSubmit, loading, error }) {
...
```jsx
// pages/RequestMoneyPage.jsx
import React from 'react';
import RequestMoneyForm from '../components/RequestMoney/RequestMoneyForm';
import { useRequestMoney } from '../hooks/useRequestMoney';

export default function RequestMoneyPage() {
  const { loading, success, error, transaction, newBalance, submit, reset } = useRequestMoney();
  ...
  return (
    <div className="page">
      <h1>طلب المال</h1>
      <RequestMoneyForm onSubmit={submit} loading={loading} error={error} />
    </div>
  );
}
```
