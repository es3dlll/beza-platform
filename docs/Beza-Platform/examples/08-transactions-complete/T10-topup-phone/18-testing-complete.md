...
```php
<?php
// tests/Feature/PhoneTopupTest.php
...
class PhoneTopupTest extends TestCase
{
    use RefreshDatabase;
    ...
    {
        $response = $this->withToken($this->token)->postJson('/topup', [
            'amount'   => 100.00,
            'currency' => 'USD',
            'pin'      => '1234',
        ]);
...
```bash
php artisan test --filter=PhoneTopupTest
