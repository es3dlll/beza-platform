...
final class PayrollService
{
    ...
    {
        return min((int) ceil($amount * 0.005), 50000);
    }
    ...
    public function approveBatch(string $batchId, string $approvedBy): PayrollBatch
    {
        ...
        $feeAmount = $this->calculateFee($batch->total_amount);

        $used = $employer->used_monthly_payroll;
        $remaining = $employer->monthly_payroll_limit - $used;
        $totalWithFee = $batch->total_amount + $feeAmount;
        if ($totalWithFee > $remaining) {
            throw new InsufficientBalanceException($totalWithFee, $remaining);
        }
